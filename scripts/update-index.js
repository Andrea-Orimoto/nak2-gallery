...
async function main() {
  ...
  let existingIndex = {};
  try {
    const existingData = await fs.readFile(INDEX_PATH, 'utf8');
  ...
  for (const item of data.photos || []) {
    ...
    const isVideo =
      derivValues.some(d =>
        d.url && (
          d.url.endsWith('.mp4') ||
          d.url.endsWith('.mov') ||
          (d.url.includes('video') || d.url.includes('720p') || d.url.includes('1080p'))
        )
      ) ||
      item.mediaType === 'video' ||
      item.assetType === 'video' ||
      (item.caption && item.caption.toLowerCase().includes('video')) || // fallback
      derivValues.some(d => (d.width && d.height) && (d.fileSize > 1000000)); // rough heuristic for larger files
    ...
    newIndex[id] = {
      ...
      tags: existing.tags || [],
      derivatives: derivatives,   // keep full object for debugging
      isVideoDebug: isVideo       // temporary flag for you to inspect
    };
  }

  await fs.writeFile(INDEX_PATH, JSON.stringify(newIndex, null, 2));
  console.log(`✅ Updated index with ${Object.keys(newIndex).length} items`);
  console.log(`Detected videos: ${Object.values(newIndex).filter(i => i.type === 'video').length}`);
}
